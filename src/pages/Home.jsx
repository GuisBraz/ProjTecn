...
import { LayoutDashboard, ClipboardList, Archive, CalendarClock, ListOrdered } from 'lucide-react'
...
export default function Home() {
  ...
  const shortcuts = [
    { key: 'dashboard', title: t('nav_dashboard'), icon: LayoutDashboard, to: '/maquina' },
    { key: 'pgms', title: t('nav_pgms_ativos'), icon: ClipboardList, to: '/maquina/pgms' },
    { key: 'programacao', title: t('nav_programacao'), icon: CalendarClock, to: '/maquina/programacao' },
    { key: 'fila', title: t('nav_fila'), icon: ListOrdered, to: '/maquina/fila' },
    { key: 'finalizados', title: t('nav_finalizados'), icon: Archive, to: '/maquina/finalizados' },
  ]
    ...
    <div style={{ padding: '40px 32px', maxWidth: '900px', margin: '0 auto' }}>
      ...
      {verMaquina ? (
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: '16px', marginTop: '32px' }}>
          {shortcuts.map(({ key, title, icon: Icon, to }) => (
            <button
              key={key}
              onClick={() => navigate(to)}
              className="card"
