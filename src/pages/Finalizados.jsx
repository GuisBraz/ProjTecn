...
import { getTempoCorteTexto } from '../lib/pgmTimers'
...
export default function Finalizados() {
  ...
  const handleReativar = async (pgm) => {
    ...
    logAction({ user: audit, action: 'reativou', pgmId: pgm.id, pgmLabel: pgm.pgm })
  ...
  return (
    <div style={{ padding: '28px' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '24px', flexWrap: 'wrap', gap: '12px' }}>
        <div>
          <h1 style={{ fontSize: '22px', fontWeight: '700' }}>Finalizados</h1>
      ...
      <div className="card" style={{ padding: 0, overflow: 'hidden' }}>
        {loading ? (
          ...
          <div className="empty-state"><Archive size={44} /><p>Nenhum PGM finalizado encontrado</p></div>
        ) : (
          <div className="table-wrap">
            <table>
              <thead>
                <tr><th>PGM</th><th>Commessa</th><th>Máquina</th><th>Peso (kg)</th><th>Material</th><th>Início Corte</th><th>Finalizado em</th><th>Tempo de Corte</th><th>Ações</th></tr>
              </thead>
              <tbody>
                {filtered.map(p => {
                  const mq = getMaquina(p.espessura_mm)
                  return (
                    <tr key={p.id}>
                      ...
                      <td style={{ color: 'var(--text-secondary)', fontSize: '12px' }}>{getTempoCorteTexto(p)}</td>
                      <td>
